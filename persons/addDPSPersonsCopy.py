...
def process_one_page(number, _size):
    persons = dps["persons"].find(persons_query, pj, no_cursor_timeout=True).skip(number * _size).limit(_size)

    with ThreadPoolExecutor(max_workers=16) as executor:
        executor.map(process_one_person, persons)

    return persons.count(True)


def process_one_person(person):
    updater = {
        "_id": person["_id"],
    ...
    for elem in regions:
        ...
        if result:
            ...
            break
    try:
        isFound = dps["xxx_dps_persons"].find_one({"_id": person["_id"]})
        if isFound:
            print(f"[SKIPPED] {person['_id']}")
            return
...
start = datetime.now()
while True:
    processed = process_one_page(page, size)
    if processed > 0:
        page += 1
    else:
        break
